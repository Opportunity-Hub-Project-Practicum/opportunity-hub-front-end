...
export default function EmployerSeekerProfilePage() {
    ...
    const loadProfile = useCallback(async () => {
        if (!seekerIdParam || Number.isNaN(seekerId)) {
            setProfile(null);
            setLoading(false);
            return;
        }
        ...
        try {
            const [profileResponse, favorites] = await Promise.all([
                fetchPublicSeekerProfile(seekerId),
                fetchFavoriteCandidates(),
            ]);
            ...
            const matchedFavorite = favorites.find((item) => item.seeker_id === seekerId);
    ...
    const handleReportSubmit = async (reportReason: string) => {
        if (Number.isNaN(seekerId) || isSubmittingReport) {
            return;
        }
        ...
        try {
            await submitSeekerProfileReport(seekerId, reportReason);
    ...
    if (loading) {
        return (
            <section className="page-container">
                <BackButton />
                <p className="text-gray-500">Loading seeker profile...</p>
            </section>
        );
    }

    if (error || Number.isNaN(seekerId)) {
        return (
            <section className="page-container">
                <BackButton />
                <p className="text-gray-600">{error ?? "Seeker not found."}</p>
            </section>
        );
    }

    const displayName = profile?.full_name ?? `Candidate #${seekerId}`;
    const displayEmail = profile?.email ?? "Not provided";
    const profileImage = profile?.profile_img ? resolveAssetUrl(profile.profile_img) : "";
        ...
        <section className="page-container">
            ...
            {isProfilePrivate && (
                <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    This seeker profile is private or unavailable.
                </div>
            )}
            ...
            <div className="mb-8 flex flex-col gap-4 border-b border-[#E4E5E8] pb-6 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-[#767F8C]">
                            {latestExperience?.job_title ?? "Seeker profile"}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    >
                        <Star
                            size={18}
                            className={favoriteCandidateId ? "fill-yellow-400 text-yellow-400" : ""}
                        />
                        {favoriteCandidateId ? "Saved" : "Save Candidate"}
                    </button>
                    ...
                    {favoriteCandidateId && (
                        <button
                            type="button"
                            onClick={() => navigate(
                                `${ROUTES.EMPLOYER.ROOT}/${ROUTES.EMPLOYER.SAVE_CANDIDATE_DETAIL(favoriteCandidateId)}`,
                            )}
                            className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-[#F8F9FA] px-4 py-2.5 text-sm font-medium text-[#18191C] transition-colors hover:bg-blue-50"
                        >
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-6 rounded-lg border border-[#E8F1FC] bg-white p-6">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-sm font-medium">
                                    {profile.educations?.[0]?.degree ?? "Not provided"}
                                </p>
                            </div>
                        </div>
